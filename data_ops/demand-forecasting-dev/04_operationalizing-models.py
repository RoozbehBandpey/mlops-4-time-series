...
prodFolderName = "prod"
forecastsFolderName = "forecasts"
prodTableName = "kaggle_prod_data_delta"
forecastsTableName = "kaggle_forecasts_data_delta"
prodTablePath = SILVER_MOUNTPOINT + f"/kaggle/{prodFolderName}"
forecastsTablePath = SILVER_MOUNTPOINT + f"/kaggle/{forecastsFolderName}"
...
# The following cell is provided to reset your predictions.
# spark.sql(f"DROP TABLE IF EXISTS {forecastsTableName}")
# dbutils.fs.rm(forecastsTablePath, True)
...
for i, training_date in enumerate( daterange(date(2017,11,30), date(2017,12,7)) ):
  ...
  with mlflow.start_run():
    ...
    (forecastDF.write
      ...
      .mode('append')
      .option("path", forecastsTablePath)
      .saveAsTable(forecastsTableName))
...
# MAGIC %md
# MAGIC We'll perform an inner join between our forecasts and the test data to add our true sales at `y`.
...
# MAGIC FROM kaggle_forecasts_data_delta a
# MAGIC INNER JOIN kaggle_test_data_delta b
