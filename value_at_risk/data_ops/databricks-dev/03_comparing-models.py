...
# MAGIC 
# MAGIC Here, we'll build our model using daily item sales aggregated across stores.
# MAGIC 
# MAGIC ### Parallelize the training Code
# MAGIC 
...
# MAGIC %md
# MAGIC We begin our process by summing our item sales across stores. Make sure you set this configuration ```spark.conf.set("spark.sql.shuffle.partitions", 500)``` with the correct number of unique items before executing your function. It should match the number of store-item pairs in our training set.
...
# MAGIC 
# MAGIC In the cell below, join the `item_store_ratios` with the predictions we made above. Save out a Delta table `agg_item_preds` with the following schema:
# MAGIC 
# MAGIC | field | type |
# MAGIC | --- | --- |
# MAGIC | store | INT |
...
tablePath = SILVER_MOUNTPOINT + f"/kaggle/{tableName}"
...
# MAGIC %sql
# MAGIC SELECT * FROM agg_item_preds

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM store_item_preds
...
# MAGIC   FROM kaggle_test_data_delta a
